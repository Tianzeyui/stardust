...
fn build_tool_registry() -> ToolRegistry {
    ...
    r.register("workspace_write_file", "Write/create file. path, content.", json!({"type":"object","properties":{"path":{"type":"string","description":"Absolute file path"},"content":{"type":"string","description":"File content"}},"required":["path","content"]}), |i: Value| { let p=resolve(i["path"].as_str().unwrap_or("")); let c=i["content"].as_str().unwrap_or("").to_string(); Box::pin(async move { tokio::fs::write(&p,&c).await.map_err(|e|format!("{e}"))?; Ok("ok".into()) }) });
    r.register("workspace_edit_file", "Edit file (full replace). path, content.", json!({"type":"object","properties":{"path":{"type":"string","description":"Absolute file path"},"content":{"type":"string","description":"File content"}},"required":["path","content"]}), |i: Value| { let p=resolve(i["path"].as_str().unwrap_or("")); let c=i["content"].as_str().unwrap_or("").to_string(); Box::pin(async move { tokio::fs::write(&p,&c).await.map_err(|e|format!("{e}"))?; Ok("ok".into()) }) });
    r.register("workspace_append_file", "Append to file. path, content.", json!({"type":"object","properties":{"path":{"type":"string","description":"Absolute file path"},"content":{"type":"string","description":"File content"}},"required":["path","content"]}), |i: Value| { let p=resolve(i["path"].as_str().unwrap_or("")); let c=i["content"].as_str().unwrap_or("").to_string(); Box::pin(async move { use std::io::Write; let mut f=std::fs::OpenOptions::new().append(true).create(true).open(&p).map_err(|e|format!("{e}"))?; f.write_all(c.as_bytes()).map_err(|e|format!("{e}"))?; Ok("ok".into()) }) });
    ...
    r.register("workspace_list_dir", "List directory. path.", json!({"type":"object","properties":{"path":{"type":"string","description":"Absolute directory path"}},"required":["path"]}), |i: Value| { let p=i["path"].as_str().unwrap_or(".").to_string(); Box::pin(async move { let mut d=tokio::fs::read_dir(&p).await.map_err(|e|format!("{e}"))?; let mut f=vec![]; while let Ok(Some(e))=d.next_entry().await { f.push(e.file_name().to_string_lossy().to_string()); } Ok(f.join("\n")) }) });
    ...
    r.register("run_terminal", "Execute shell command. command, cwd (optional).", json!({"type":"object","properties":{"command":{"type":"string","description":"Shell command to execute"},"cwd":{"type":"string","description":"Working directory (optional)"}},"required":["command"]}), |i: Value| { let cmd=i["command"].as_str().unwrap_or("").to_string(); let cwd=i["cwd"].as_str().unwrap_or(".").to_string(); Box::pin(async move { match tokio::process::Command::new("/bin/sh").args(["-c",&cmd]).current_dir(&cwd).output().await { Ok(o)=>Ok(format!("{}{}",String::from_utf8_lossy(&o.stdout),String::from_utf8_lossy(&o.stderr))), Err(e)=>Ok(format!("{e}")) } }) });
