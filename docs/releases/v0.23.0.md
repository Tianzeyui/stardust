# v0.23.0 — Code Agent 全家桶

## 文件操作

- `workspace_write_file` — 创建/覆写文件，工作区外需用户确认
- `workspace_append_file` — 追加内容到文件末尾
- `workspace_delete_file` — 删除文件
- `workspace_create_dir` — 创建目录（含父目录）
- `workspace_edit_file` — 精确字符串替换，对标 Claude Code Edit，支持唯一匹配 + replace_all
...
- `git_add` / `git_reset` / `git_commit`（需用户确认）/ `git_branch` / `git_checkout` / `git_push`
