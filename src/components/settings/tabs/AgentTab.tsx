    ...
    <div className="w-full space-y-6">
      <fieldset className="rounded-lg border border-border p-4">
        ...
        <div className="flex items-center gap-2">
          <button className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${memoryEnabled ? 'bg-primary' : 'bg-muted'}`} onClick={() => { const v = !memoryEnabled; setMemoryEnabled(v); saveMemoryEnabled(v) }}>
      ...
      <fieldset className="rounded-lg border border-border p-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Token 上限</Label><p className="text-[10px] text-muted-foreground/50 mb-1">0=自动，用模型默认值</p><div className="flex items-center gap-1"><Input type="number" min={0} step={1024} value={tokenLimit} onChange={e => { const v = parseInt(e.target.value) || 0; setTokenLimit(v); saveTokenLimit(v) }} className="h-8 w-24 text-center text-sm" /><span className="text-xs text-muted-foreground">tok</span></div></div>
